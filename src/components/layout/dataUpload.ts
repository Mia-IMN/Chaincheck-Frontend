...
export async function uploadBlogContent(content: string, epochs: number = 5): Promise<string | null> {
    try {
        ...
        const data = response.data as { blob_id: string };
...
export async function uploadBlogPost(blogData: {
    title: string;
    content: string;
    author: string;
    excerpt: string;
...
}, epochs: number = 5): Promise<string | null> {
    try {
        const blogPost = {
            ...blogData,
